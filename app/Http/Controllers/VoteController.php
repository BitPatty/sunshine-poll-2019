...
class VoteController extends Controller
{
  ...
  public function create(Request $request)
  {
    ...
    if ($validator->fails()) {
      ...
      return response(view('Home', ['errorMessages' => $this->parseValidationErrors($validator), 'selectedValues' => $params]), 412);
    }

    $src_profile = $this->fetchSRCProfile($params['src_token']);

    if ($src_profile == null) {
      return response(view('Home', ['errorMessages' => [Messages::FAIL_RETRIEVE_PROFILE], 'selectedValues' => $params]), 400);
    } else {
      $dbVote = Vote::where(['src_id' => $src_profile['id']])->first();

      if (!$dbVote) {
        if (isset($params['custom_run_url']) || $request->post('submitting_custom_run') === '1') {
          $dbVote = new Vote();
        } else {
          $infoMessages = [Messages::MISSING_RUN_UPDATE];
          return response(view('Home', ['infoMessages' => $infoMessages, 'missingRun' => true, 'selectedValues' => $params]));
        }
      } else if ($dbVote->has_src_run == false && $params['custom_run_url'] == null && $request->post('submitting_custom_run') !== '1') {
      ...
      foreach (Questions::QUESTION_LIST as $question) {
   ...
   * @return Returns the profile or NULL if the format wasn't as expected
   */
